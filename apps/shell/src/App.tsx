...
import { LibraryModuleBoundary } from './features/LibraryModuleBoundary';
...
export function App() {
  ...
  const snapshot: LibrarySnapshot = useMemo(() => ({
    catalog: CATALOG,
    saved: workbench.saved.map(({ source, residueCount }) => ({
      id: source.id, filename: source.filename, residueCount,
      label: source.origin === 'local' ? source.filename : source.id,
    })),
    currentId: workbench.workspace?.source.id ?? null,
    panelOpen: libraryOpen,
    searchOpen,
  }), [workbench.saved, workbench.workspace?.source.id, libraryOpen, searchOpen]);
  const libraryHost: LibraryHost = {
    openPublic(id) { void workbench.loadPublic(id); setLibraryOpen(false); setSearchOpen(false); },
    openSaved(id) {
      const item = workbench.saved.find(({ source }) => source.id === id);
      if (!item) { workbench.setError('This structure is no longer in the device library. Import it again or open its public PDB identifier.'); return; }
      void workbench.loadSaved(item.source); setLibraryOpen(false); setSearchOpen(false);
    },
    requestImport: () => input.current?.click(),
    removeSaved: (id) => void remove(id),
    clearSaved: () => void remove(),
    closeSearch: () => setSearchOpen(false),
    reportError: workbench.setError,
  };
  return <div className="app-shell" onDragOver={(event) => {
    ...
    <div className="app-body">
      <LibraryModuleBoundary snapshot={snapshot} host={libraryHost} recovery={(message, retry) => <>
        <LibraryPanel saved={workbench.saved} currentId={workbench.workspace?.source.id} open={libraryOpen}
          onLoad={libraryHost.openPublic} onLoadSaved={(source) => libraryHost.openSaved(source.id)}
          onRemove={libraryHost.removeSaved} onClear={libraryHost.clearSaved} onImport={libraryHost.requestImport}
          notice={<div className="library-recovery"><p role="alert"><strong>Library recovery mode.</strong> The Vue module could not run: {message}</p>
            <p>Basic controls remain available. Retry, or reload when your connection is restored.</p>
            <button className="text-button" onClick={retry}>Retry Vue library</button></div>} />
        <SearchDialog open={searchOpen} onClose={libraryHost.closeSearch} onLoad={libraryHost.openPublic} recoveryMessage={message} />
      </>} />
